import { vaultRepo, VaultChunk } from "@/lib/db";
...
/** Hybrid local retrieval: BM25 lexical rank + vector rank + RRF fusion + deterministic reranker. */
export async function searchVault(
  ...
  // Build candidates for hybrid reranker — after RRF / fusion stage before returning
  ...
  for (const [chunkId, fusedScore] of fused.entries()) {
    ...
    const strategy: Candidate["strategy"] = lexicalSignal && semanticSignal ? "hybrid" : semanticSignal ? "vector" : "lexical";
    candidates.push({
      id: String(chunkId),
      docId: chunk.docId,
  ...
  // Integrate rerankVaultChunks — hybrid scoring with provenance generation
  const reranked = rerankVaultChunks(trimmed, candidates, k);

  // Normalize final score for VaultSearchHit contract while preserving rerankScore
